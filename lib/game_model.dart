...
class GameModel with ChangeNotifier {
  ...
  GameModel(int size) {
    gridSize = size;
    _grid = List.generate(
        gridSize, (i) => List.generate(gridSize, (j) => 0)); // 将网格初始化为全 0
  ...
  void _addNewTile() {
    ...
    if (emptyTiles.isNotEmpty) {
      // 如果有空白位置
      int index =
          emptyTiles[Random().nextInt(emptyTiles.length)]; // 随机选择一个空白位置索引
      _grid[index ~/ gridSize][index % gridSize] =
          Random().nextInt(10) == 0 ? 4 : 2; // 在该位置插入 2 或 4（10% 概率为 4）
    }
  }
  ...
  void _rotateLeft() {
    List<List<int>> newGrid = List.generate(
        gridSize, (i) => List.generate(gridSize, (j) => 0)); // 创建一个新的空网格
    for (int i = 0; i < gridSize; i++) {
      for (int j = 0; j < gridSize; j++) {
  ...
  bool _slideLeft() {
    ...
    for (int i = 0; i < gridSize; i++) {
      ...
      for (int j = 0; j < gridSize; j++) {
        if (_grid[i][j] != 0) {
          // 如果当前格子不为空
          if (newRow[targetIndex] == 0) {
            // 如果目标位置为空
            newRow[targetIndex] = _grid[i][j]; // 将当前方块移动到目标位置
          } else if (newRow[targetIndex] == _grid[i][j]) {
            // 如果目标位置的方块与当前方块相同
      ...
      if (!moved && !_equality.equals(newRow, _grid[i])) {
        // 使用 ListEquality 比较行内容是否改变
        moved = true; // 标记为有移动
      }
